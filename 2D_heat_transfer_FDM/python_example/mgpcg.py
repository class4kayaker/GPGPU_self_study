...
class ALoc:
    ...
    def setStencil(self):
        self.cen[:, :] = 4.*self.n**2
        self.xdir[:, :] = -1.*self.n**2
        self.ydir[:, :] = -1.*self.n**2
        self.xpydir[:, :] = 0.
        self.xmydir[:, :] = 0.
    ...
    def setFromK(self, k):
        ...
        # Set diag elements
        self.cen[:, :] = 0.
        self.cen += 4. * k[1:-1, 1:-1] * denom
        self.cen += 1. * k[1:-1, 0:-2] * denom  # k_x-
        self.cen += 1. * k[1:-1, 2:] * denom  # k_x+
        self.cen += 1. * k[0:-2, 1:-1] * denom  # k_y-
        self.cen += 1. * k[2:, 1:-1] * denom  # k_y+
        # Set x-dir
    ...
    def jacobi(self, u, f, weight=1.):
        offdiag = self.mult(u) - self.cen * u
        return (1.-weight)*u+weight*(f-offdiag)/self.cen
            ...
            for ioff in range(3):
                ...
                for j in range(joff, newN-1, 3):
                    ...
                    for i in range(ioff, newN-1, 3):
                        ...
                        if i < newN-2:
                            # X-Dir
                            ret.xdir[j, i] = (v[jst:jen, ist+2:ien+2] *
                                              fSten2d).sum()

                        if j < newN-2:
                            # Y-Dir
                            ret.ydir[j, i] = (v[jst+2:jen+2, ist:ien] *
                                              fSten2d).sum()

                        if i < newN-2 and j < newN-2:
                            # XpY-Dir
                            ret.xpydir[j, i] = (v[jst+2:jen+2, ist+2:ien+2] *
                                                fSten2d).sum()

                        if i < newN-2 and j > 0:
                            # XmY-Dir
                            ret.xmydir[j-1, i] = (v[jst-2:jen-2, ist+2:ien+2] *
                                                  fSten2d).sum()
...
class Grid2d:
    ...
    def jacobi(self, count, weight=1.):
        n = 0
        while(n < count):
...
def init_2d(u, f):
    snx, sny = u.shape
    nx, ny = snx+1, sny+1
    xh = np.linspace(0., 1., nx+1)
    yh = np.linspace(0., 1., ny+1)
    for yi in range(ny-1):
        for xi in range(nx-1):
            xl = xh[xi+1]
            yl = yh[yi+1]
            val = - 2.*((1.-6.*xl**2.)*yl**2*(1.-yl**2.)
                        + (1.-6.*yl**2.)*xl**2*(1.-xl**2.))
...
def initP2(k, f):
    ...
    nd8 = n/8
    k[:, :] = 1.
    k[5*nd8+1:n-nd8, nd8+1:n-nd8] = 100.
    k[nd8+1:5*nd8+1, 3*nd8+1:5*nd8] = 100.
    f[:, :] = -80
    f[:nd2-1, :nd2-1] = 80
    f[nd2-1, :] = 0
    f[:, nd2-1] = 0
    ...
    for l in llist:
        ...
        for prob in probs:
            ...
            for meth in meths:
                u[:, :] = 0.
                if meth == 'MGCG':
                    niter, normR, ctime = mgcg(mgg, u, f, mxiters,
                                               tol, swtype=swtype)
                elif meth == 'MG':
                    niter, normR, ctime = mgrun(mgg, u, f, mxiters,
                                                tol, swtype=swtype)
