...
void do_computation(const FCT_initialization::ProblemConfig &config,
  ...
  std::vector<double> host_u_state(ndx + 4);
  std::vector<double> host_flux_low(ndx + 1);
  std::vector<double> host_flux_high(ndx + 1);
  std::vector<double> host_adiff_flux(ndx + 1);
  std::vector<double> host_flux_c(ndx + 1);
  ...
  {
    // Device buffers
    cl::sycl::buffer<double, 1> u_state(host_u_state.data(), state_size);
    cl::sycl::buffer<double, 1> flux_low(host_flux_low.data(), flux_size);
    cl::sycl::buffer<double, 1> flux_high(host_flux_high.data(), flux_size);
    cl::sycl::buffer<double, 1> adiff_flux(host_adiff_flux.data(), flux_size);
    cl::sycl::buffer<double, 1> flux_c(host_flux_c.data(), flux_size);
